...
class ProceduresSearch extends React.Component {
  ...
  render() {
    ...
    const results = this.state.procedures.filter( proc => {
      ...
      // return true, will be included in the output array
    ...
    return (
      <div>
        <h3>Procedure Results for "{this.props.match.params.query}"</h3>
        <ul>
          {
            results.map(r => (  // parenthesis to divide statement in several lines
              <li key={r.name}>
              {r.name} (${r.cost})
              </li>
            ))
