...
app.animateParticles = () => {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    // Calculate the distnace(squared) of each particle from the
    // earth. Because the earth is at (0,0,0) our calculatin is
    // simplified and only needs to use the particle's position
    const distSquared = (p.x)**2 + (p.y)**2 + (p.z)**2;
  ...
  // app.particleSystem.rotation.y += app.controls.rotationSpeed*0.1;
